...
- 增量更新：按缺失交易日逐日调用 `query_all_stock(day)` 和 `query_daily_history_k_AStock(date)`。即使一年未更新，也只需按约 240 个交易日调用全市场日线接口，而不是逐只股票查询。
...
BaoStock 的全市场接口实际会把部分停牌证券的 `volume`、`amount` 返回为空。程序仅在 `tradestatus=0` 时把这两个空值规范化为 0，随后仍强制检查停牌 OHLC 相等且量额为 0；正常交易证券出现空量额会立即中止。
...
首次启动会执行全量历史导入，耗时取决于股票数和 BaoStock 响应速度。每只股票成功后都会独立提交并记录进度；程序重启后会从未完成股票继续。
...
测试覆盖 BaoStock 错误响应、字段和日期错误、重复数据、停牌规则、股票集合不一致、前复权因子、事务回滚、首次导入恢复、致命标记、只读 SQL 和真实 Arrow Flight 客户端/服务端通信。
