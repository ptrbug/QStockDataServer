...
def test_initial_import_incremental_factor_and_snapshot(app_config) -> None:
    ...
    try:
        ...
        assert qfq_columns == [
            ...
            "raw_high", "raw_low", "raw_close", "raw_preclose", "volume",
            "amount", "turn", "pct_chg", "trade_status",
        ]
        raw_prices = snapshot.connection.execute(
            "SELECT raw_open, raw_high, raw_low, raw_close, raw_preclose FROM daily_qfq "
            "WHERE symbol='sh.600000' AND date='2024-01-05'"
        ).fetchone()
        assert raw_prices == (
            pytest.approx(6.8), pytest.approx(7.0), pytest.approx(6.7),
            pytest.approx(6.9), pytest.approx(6.88),
        )
